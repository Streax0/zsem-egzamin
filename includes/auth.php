...
if (!function_exists('requireLogin')) {
    function requireLogin(bool $allowGuest = false) {
        ...
        if (!isLoggedIn()) {
            ...
            echo '<style>body.auth-page{display:flex;align-items:center;justify-content:center;min-height:100vh;background:linear-gradient(135deg,#f8fafc 0%,#e2e8f0 100%);margin:0;font-family:"Inter",sans-serif}.blocked-card{background:#fff;border-radius:1rem;box-shadow:0 10px 25px rgba(0,0,0,0.05);padding:2.5rem;max-width:500px;width:100%;text-align:center}.blocked-icon{font-size:4rem;color:#0d6efd;margin-bottom:1rem}.blocked-badge{display:inline-block;padding:0.35em 0.65em;font-size:0.75em;font-weight:700;line-height:1;color:#fff;text-align:center;white-space:nowrap;vertical-align:baseline;border-radius:50rem;background-color:#dc3545;margin-bottom:1rem}.btn-action{font-weight:600;padding:0.75rem 1.5rem;border-radius:0.5rem;transition:all 0.2s}</style>';
            echo '</head><body class="auth-page"><main class="blocked-card mx-3" role="main">';
            echo '<div class="blocked-icon"><i class="bi bi-shield-lock"></i></div>';
            echo '<span class="blocked-badge">Dostęp ograniczony</span>';
            echo '<h1 class="h3 fw-bold mb-3">Wymagane logowanie</h1>';
            echo '<p class="text-muted mb-4">Aby uzyskać dostęp do tej strony, musisz się zalogować. Jeśli nie masz konta, możesz je założyć.</p>';
            ...
            echo '<a class="btn btn-outline-secondary btn-action" href="' . htmlspecialchars($home_url, ENT_QUOTES) . '"><i class="bi bi-house me-2"></i>Strona główna</a>';
            echo '</div>';
            echo '</main>';
            echo '</body></html>';
            exit();
