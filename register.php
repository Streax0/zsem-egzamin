...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username !== '' && (mb_strlen($username, 'UTF-8') < 3 || mb_strlen($username, 'UTF-8') > 50 || !preg_match('/^[A-Za-z0-9_.-]{3,50}$/', $username))) {
        $errors[] = 'Login musi mieć 3-50 znaków i może zawierać litery, cyfry, kropkę, myślnik oraz podkreślenie.';
    }
    ...
    if (empty($errors)) {
        ...
        if (empty($errors)) {
            $newUserId = register($username, $email, $password, $first_name, $last_name, $classParts['year'], $classParts['suffix']);
            if ($newUserId) {
                if ($apply_teacher) {
    ...
    <main class="auth-form-panel register-form-panel" role="main">
        ...
        <form method="POST" id="registerForm">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="username" id="regUsername" class="form-control" placeholder="Puste = imię.nazwisko" value="<?= htmlspecialchars($username) ?>" minlength="3" maxlength="50" pattern="[A-Za-z0-9_.-]{3,50}" autocomplete="username">
