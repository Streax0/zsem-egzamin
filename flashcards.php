...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateProgress() {
        ...
        if (!target) return;
        target.innerHTML = qualificationProgress() || '<div class="small text-muted">Brak danych postępu dla kwalifikacji.</div>';
    }
    function render() {
        const card = pool[index];
        const box = document.getElementById('flashcardCard');
        if (!card) {
            box.innerHTML = '<strong>Brak fiszek</strong><p>Zmień filtr albo wróć później po zatwierdzeniu nowych kart.</p>';
            return;
        }
    ...
    document.addEventListener('keydown', e => {
        ...
        if (e.key === 'ArrowLeft') { e.preventDefault(); rate('hard', 'left'); }
        if (e.key === 'ArrowRight') { e.preventDefault(); rate('easy', 'right'); }
        if (e.key === 'ArrowDown') { e.preventDefault(); rate('medium', 'right'); }
        if (e.key === 'ArrowUp' || e.key === ' ') { e.preventDefault(); flipped = !flipped; render(); }
    });
